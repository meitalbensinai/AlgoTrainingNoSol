...
def create_terrorists_places():
    ...
    places = []
    for i in range(10):
        places.append(generate_random_point(LONG_LAT_LEBANON))
    return places
...
def create_grand_parents():
    ...
    for i in range(curr_id, curr_id + GRAND_PARENTS):
        ...
        birth_date = "%02d/%02d/%s" % (birth_date.month, birth_date.day, str(birth_date.year)[2:])
...
def create_parents(grand_parents):
    ...
    for i in range(0, len(grandparents_ids), 2):
        ...
        for child in range(childrens_num):
            ...
            birth_date = datetime.datetime(2016, 8, 1) - \
                datetime.timedelta(days = age*365)
            birth_date = "%02d/%02d/%s" % (birth_date.day, birth_date.month, str(birth_date.year)[2:])
            if random.random() < 0.1:
                birth_date = "01/01/1000"
...
def create_childrens(parents):
    ...
    for i in range(0, len(parents_ids)-1, 2):
        ...
        for child in range(childrens_num):
            ...
            birth_date = datetime.datetime(2016, 8, 1) - \
                datetime.timedelta(days = age*365)
            birth_date = "%02d/%02d/%s" % (birth_date.day, birth_date.month, str(birth_date.year))
            if random.random() < 0.1:
                birth_date = "01/01/1000"
...
def delete_dict_values(x_dict, p):
    keys_to_remove = random.sample(x_dict.keys(), int(p*len(x_dict)))
    for key in keys_to_remove:
        del x_dict[key]

def create_db_1(grand_parent, parents, childrens):
    delete_dict_values(grand_parents, 0.2)
    delete_dict_values(parents, 0.2)
...
for i in range(5000):
    ...
    if coin_flip > 0.9: # generate random point
        arrest_place = generate_random_point(LONG_LAT_LEBANON)
    else: # one of terrorist places
        arrest_place = random.choice(TERRORIST_PLACES)
    id_prefix = "%02d" % (random.choice(range(1,23)))
    arrests_table[id_prefix + str(candidates[i])] = arrest_place
...
for suspect_id in suspects_list:
    if random.random() < 0.9: # unemployed
        pass
    else: # employed
...
salaries_df = pd.DataFrame({"suspect_id": [tup[0] for tup in salaries_list],
                            "annual_salary": [tup[1]/12.0 for tup in salaries_list]})
